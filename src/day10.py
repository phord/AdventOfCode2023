...
def getem():
    return '''F---7-FJ.7--FJ7.-.7-|7F.|7FJ77F7J-|--.FLF|.F-7-F7.F.J77|--|7--|7|-J-F--7-F7.F7-F7F7F-|-|-F777L7|-.FLF|-77F7-FJF|JF--F.77J-L7-FF--7.FFF7FF7.7
...
# directions we can enter a pipe
...
for y in range(height):
    moves.append(['   ' for i in range(width)])
    moves.append(['   ' for i in range(width)])
    moves.append(['   ' for i in range(width)])

def valid_move(direction, pos):
    x,y = pos
    if x < 0 or y < 0:
        return False
    ...
    valid = directions[pipe]
    # print(f"  ===   {pos} {x},{y} {pipe}: {valid}  {direction}")

    if direction not in valid:
        return False

    nx,ny = [x for x in valid if x != direction][0]
    return (-nx, -ny)
...
def move(start, direction):
    ...
    next = valid_move(direction, (x,y))
    # print(f"Valid: {next} from {(start)} to ({x},{y}) in {direction}")
    if not next:
        return False

    for i in range(3):
        moves[y*3+i][x] = shapes[grid[y][x]][i]
...
def flood(pos):
    ...
    while pos:
        ...
        if x < 0 or y < 0 or x >= len(moves[0]) or y >= len(moves):
            continue
        ...
        if (x+1,y) not in done: pos.append((x+1,y))
        if (x-1,y) not in done: pos.append((x-1,y))
        if (x,y+1) not in done: pos.append((x,y+1))
        if (x,y-1) not in done: pos.append((x,y-1))


moves = [[*''.join(line)] for line in moves]
flood([(0,0)])
